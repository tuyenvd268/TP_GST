...
def train(model, data_loader, valid_loader, optimizer, scheduler, batch_size=32, ckpt_dir=None, writer=None, DEVICE=None):
    ...
    while global_step < args.max_step:
        epoch_loss, epoch_loss_mel, epoch_loss_fmel, epoch_loss_ff, epoch_loss_se = 0., 0., 0., 0., 0.
        ...
        if args.log_mode:
            ...
            avg_loss_se = epoch_loss_se / (len(data_loader))
            avg_loss = loss / (len(data_loader))
            
            log = {
                "total": avg_loss, 
                "mel_loss": avg_loss_mel,
                "loss_se": avg_loss_se,
                "loss_ff": avg_loss_ff,
            }
            print("\n###epoch log: \n", log)
            ...
            mel = mels[0:1].transpose(1, 2).cpu().detach().numpy()
